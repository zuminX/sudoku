...
public interface GameRecordConvert {
  ...
  /**
   * 将游戏记录传输层对象转换为对应实体类对象
   * <p>
   * 将数独数据中的数独矩阵和题目空缺数组转换为字符串
   *
   * @param gameRecord 提交数独信息传输层对象
   * @return 游戏记录表对应的实体类对象
   */
  @Mapping(target = "sudokuMatrix", expression = "java(PublicUtils.compressionIntArray(gameRecord.getSudokuDataBO().getMatrix()))")
  @Mapping(target = "sudokuHoles", expression = "java(PublicUtils.compressionBoolArray(gameRecord.getSudokuDataBO().getHoles()))")
